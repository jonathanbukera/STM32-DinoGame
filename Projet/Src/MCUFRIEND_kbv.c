...
void     LCD_DrawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) { LCD_FillRect(x, y, 1, h, color); }
void     LCD_DrawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) { LCD_FillRect(x, y, w, 1, color); }
void     LCD_FillScreen(uint16_t color)                                     { LCD_FillRect(0, 0, _width, _height, color); }
uint16_t LCD_Color565(uint8_t r, uint8_t g, uint8_t b) { return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | ((b & 0xF8) >> 3); }
uint16_t LCD_ReadPixel(int16_t x, int16_t y) { uint16_t color; LCD_ReadGRAM(x, y, &color, 1, 1); return color; }

uint16_t _lcd_xor, _lcd_capable;
uint16_t _lcd_ID, _lcd_rev, _lcd_madctl, _lcd_drivOut, _MC, _MP, _MW, _SC, _EC, _SP, _EP;
	
int16_t
    WIDTH=TFTWIDTH,          ///< This is the 'raw' display width - never changes
    HEIGHT=TFTHEIGHT;         ///< This is the 'raw' display height - never changes
int16_t
    _width=TFTWIDTH,         ///< Display width as modified by current rotation
    _height=TFTHEIGHT,        ///< Display height as modified by current rotation
...
uint16_t
    ...
    textbgcolor=0x0000;    ///< 16-bit text color for print()
uint8_t
    textsize_x=3,      ///< Desired magnification in X-axis of text to print()
    textsize_y=3,      ///< Desired magnification in Y-axis of text to print()
    rotation=0;       ///< Display rotation (0 thru 3)
bool
    wrap=0,           ///< If set, 'wrap' text at right edge of display
    _cp437=0;         ///< If set, use correct CP437 charset (default is off)
...
void LCD_Printf(const char *fmt, ...)
{
        ...
        while(*p) {
                if (*p == '\n') {
                        ...
                        LCD_DrawChar(cursor_x, cursor_y, *p, textcolor, textbgcolor, textsize_x);
                        cursor_x += textsize_x*6;
                        if (wrap && (cursor_x > (TFTWIDTH - textsize_x*6))) {
                                cursor_y += textsize_y*8;
                                cursor_x = 0;
                        }
                }
...
void LCD_Reset(void)
{
    ...
    RESET_IDLE;
    delay(100);
	LCD_WriteCmdData(0xB0, 0x0000);   //R61520 needs this to read ID
}
...
void LCD_WriteCmdData(uint16_t cmd, uint16_t dat) { writecmddata(cmd, dat); }
...
void LCD_PushCommand(uint16_t cmd, uint8_t * block, int8_t N) { WriteCmdParamN(cmd, N, block); }
...
{
    uint16_t h = LCD_ReadReg(reg, 0);
    uint16_t l = LCD_ReadReg(reg, 1);
...
{
    uint16_t h = LCD_ReadReg(reg, 0);
    uint16_t m = LCD_ReadReg(reg, 1);
    uint16_t l = LCD_ReadReg(reg, 2);
...
uint16_t LCD_ReadID(void)
{
    uint16_t ret;
    uint8_t msb;
    ret = LCD_ReadReg(0,0);           //forces a LCD_reset() if called before begin()
    ...
    if (ret == 0x8989)          //SSD1289 is always 8989
        return 0x1289;
    ret = LCD_ReadReg(0x67,0);        //HX8347-A
    if (ret == 0x4747)
        return 0x8347;
//#if defined(SUPPORT_1963) && USING_16BIT_BUS 
    ret = LCD_ReadReg32(0xA1);      //SSD1963: [01 57 61 01]
    if (ret == 0x6101)
        return 0x1963;
...
//#endif
    ...
    if (ret == 0x1400)          //?RM68140:[xx FF 68 14 00] not tested yet
        return 0x6814;
    ret = LCD_ReadReg32(0xD4);
    if (ret == 0x5310)          //NT35310: [xx 01 53 10]
        return 0x5310;
    ret = LCD_ReadReg32(0xD7);
    if (ret == 0x8031)          //weird unknown from BangGood [xx 20 80 31] PrinceCharles
        return 0x8031;
    ret = LCD_ReadReg40(0xEF);      //ILI9327: [xx 02 04 93 27 FF] 
    if (ret == 0x9327)
        return 0x9327;
    ret = LCD_ReadReg32(0xFE) >> 8; //weird unknown from BangGood [04 20 53] 
    if (ret == 0x2053)
        return 0x2053;
    uint32_t ret32 = LCD_ReadReg32(0x04);
    msb = ret32 >> 16;
    ret = ret32;	
//    if (msb = 0x38 && ret == 0x8000) //unknown [xx 38 80 00] with D3 = 0x1602
    if (msb == 0x00 && ret == 0x8000) { //HX8357-D [xx 00 80 00]
        ...
        LCD_PushCommand(0xB9, cmds, 3);
        msb = LCD_ReadReg(0xD0,0);
...
//        return 0x1526;          //subsequent begin() enables Command Access
    ...
    if (ret == 0xAC11)          //?unknown [xx 61 AC 11]
        return 0xAC11;
    ret = LCD_ReadReg32(0xD3);      //for ILI9488, 9486, 9340, 9341
    msb = ret >> 8;
...
/*
	...
	LCD_PushCommand(0xD9, &msb, 1);
	ret2 = LCD_ReadReg(0xD3);
    msb = 0x13;
	LCD_PushCommand(0xD9, &msb, 1);
	ret = (ret2 << 8) | LCD_ReadReg(0xD3);
//	if (ret2 == 0x93)
    	return ret2;
*/
	return LCD_ReadReg(0,0);          //0154, 7783, 9320, 9325, 9335, B505, B509
}
...
int16_t LCD_ReadGRAM(int16_t x, int16_t y, uint16_t * block, int16_t w, int16_t h)
{
    ...
    while (n > 0) {
        if (!(_lcd_capable & MIPI_DCS_REV1)) {
            LCD_WriteCmdData(_MC, x + col);
            LCD_WriteCmdData(_MP, y + row);
        }
        CS_ACTIVE;
        WriteCmd(_MR);
        setReadDir();
        ...
        while (n) {
            if (_lcd_capable & READ_24BITS) {
                ...
                if (_lcd_capable & READ_BGR)
                    ret = LCD_Color565(b, g, r);
                else
                    ret = LCD_Color565(r, g, b);
            } else {
                READ_16(ret);
...
//	    if (_lcd_ID	== 0x1963) WriteCmdParamN(0x13, 0, NULL);   //NORMAL mode
    ...
    // cope with 9320 variants
        ...
        case 0x9226:
            ...
            GS = (val & 0x80) ? (1 << 9) : 0;
            SS_v = (val & 0x40) ? (1 << 8) : 0;
            LCD_WriteCmdData(0x01, GS | SS_v | 0x001C);       // set Driver Output Control
            goto common_ORG;
#endif
        ...
        case 0x0154:
          ...
          common_S6D:
            ...
            LCD_WriteCmdData(0x01, GS | SS_v | ((_lcd_ID == 0x0139) ? 0x27 : 0x28));
            goto common_ORG;
#endif
        case 0x5420:
        case 0x7793:
		...
		case 0xB509:
            ...
            LCD_WriteCmdData(0x400, GS | NL);
            goto common_SS;
        default:
            _MC = 0x20, _MP = 0x21, _MW = 0x22, _SC = 0x50, _EC = 0x51, _SP = 0x52, _EP = 0x53;
            GS = (val & 0x80) ? (1 << 15) : 0;
            LCD_WriteCmdData(0x60, GS | 0x2700);    // Gate Scan Line (0xA700)
          common_SS:
            SS_v = (val & 0x40) ? (1 << 8) : 0;
            LCD_WriteCmdData(0x01, SS_v);     // set Driver Output Control
          common_ORG:
            ORG = (val & 0x20) ? (1 << 3) : 0;
#ifdef SUPPORT_8230
            ...
            LCD_WriteCmdData(0x03, _lcd_madctl);    // set GRAM write direction and BGR=1.
            break;
#ifdef SUPPORT_1289
        case 0x1289:
            ...
            LCD_WriteCmdData(0x01, _lcd_drivOut);   // set Driver Output Control
            LCD_WriteCmdData(0x11, ORG | 0x6070);   // set GRAM write direction.
            break;
#endif
		}
    }
    ...
    LCD_SetAddrWindow(0, 0, width() - 1, height() - 1);
    LCD_VertScroll(0, HEIGHT, 0);   //reset scrolling after a rotation
}
...
void LCD_SetAddrWindow(int16_t x, int16_t y, int16_t x1, int16_t y1)
{
    ...
    if (_lcd_capable & MIPI_DCS_REV1) {
        ...
        LCD_WriteCmdData(_MC, x);
        LCD_WriteCmdData(_MP, y);
        if (!(x == x1 && y == y1)) {  //only need MC,MP for drawPixel
            ...
            LCD_WriteCmdData(_SC, x);
            LCD_WriteCmdData(_SP, y);
            LCD_WriteCmdData(_EC, x1);
            LCD_WriteCmdData(_EP, y1);
        }
    }
}
...
//        if (offset == 0 && rotation > 1) vsp = top + scrollines;   //make non-valid
    ...
    switch (_lcd_ID) {
    case 0x7783:
        LCD_WriteCmdData(0x61, _lcd_rev);   //!NDL, !VLE, REV
        LCD_WriteCmdData(0x6A, vsp);        //VL#
        break;
#ifdef SUPPORT_0139 
    case 0x0139:
        LCD_WriteCmdData(0x07, 0x0213 | (_lcd_rev << 2));  //VLE1=1, GON=1, REV=x, D=3
        LCD_WriteCmdData(0x41, vsp);  //VL# check vsp
        break;
#endif
    ...
    case 0x9226:
	case 0x0154:
        LCD_WriteCmdData(0x31, sea);        //SEA
        LCD_WriteCmdData(0x32, top);        //SSA
        LCD_WriteCmdData(0x33, vsp - top);  //SST
        break;
#endif
#ifdef SUPPORT_1289
    case 0x1289:
        LCD_WriteCmdData(0x41, vsp);        //VL#
        break;
#endif
	case 0x5420:
    case 0x7793:
	case 0x9326:
	case 0xB509:
        LCD_WriteCmdData(0x401, (1 << 1) | _lcd_rev);       //VLE, REV 
        LCD_WriteCmdData(0x404, vsp);       //VL# 
        break;
    default:
        // 0x6809, 0x9320, 0x9325, 0x9335, 0xB505 can only scroll whole screen
        LCD_WriteCmdData(0x61, (1 << 1) | _lcd_rev);        //!NDL, VLE, REV
        LCD_WriteCmdData(0x6A, vsp);        //VL#
        break;
    }
}
...
void LCD_InvertDisplay(bool i)
{
    ...
    case 0x9226:
    case 0x0154:
        LCD_WriteCmdData(0x07, 0x13 | (_lcd_rev << 2));     //.kbv kludge
        break;
#ifdef SUPPORT_1289
    case 0x1289:
        _lcd_drivOut &= ~(1 << 13);
        if (_lcd_rev)
            _lcd_drivOut |= (1 << 13);
        LCD_WriteCmdData(0x01, _lcd_drivOut);
        break;
#endif
	case 0x5420:
    case 0x7793:
    case 0x9326:
	case 0xB509:
        LCD_WriteCmdData(0x401, (1 << 1) | _lcd_rev);       //.kbv kludge VLE 
        break;
    default:
        LCD_WriteCmdData(0x61, _lcd_rev);
        break;
    }
}
...
//        };
    ...
    case 0x8031:      //Unknown BangGood thanks PrinceCharles
        ...
        static const uint8_t FK8031_regValues[]  = {
            // 0xF2:8.2 = SM, 0xF2:8.0 = REV. invertDisplay(), LCD_VertScroll() do not work
...
#define GAMMA9486 4
    ...
    LCD_SetRotation(0);             //PORTRAIT
    LCD_InvertDisplay(false);
#if defined(SUPPORT_9488_555)
    if (_lcd_ID == 0x9488) {
		is555 = 0;
		LCD_DrawPixel(0, 0, 0xFFE0);
		if (LCD_ReadPixel(0, 0) == 0xFF1F) {
			uint8_t pixfmt = 0x06;
			LCD_PushCommand(0x3A, &pixfmt, 1);
